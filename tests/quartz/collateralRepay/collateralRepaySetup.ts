...
export const setupTestEnvironment = async () => {
  ...
  const solSpotMarketVaultAccountInfo = await connection.getAccountInfo(
    new PublicKey("DfYCNezifxAEsQbAJ1b3j6PX3JVBe8fu11KBhxsbw5d2")
  );
  const usdcSpotMarketVaultAccountInfo = await connection.getAccountInfo(
    new PublicKey("GXWqPpjQpdz7KZw9p7f5PX2eGxHAhvpNXiviFkAB8zXg")
  );
  const solSpotMarketAccountInfo = await connection.getAccountInfo(
    DRIFT_SPOT_MARKET_SOL
  );
  const usdcSpotMarketAccountInfo = await connection.getAccountInfo(
    DRIFT_SPOT_MARKET_USDC
  );
...
// export async function executecollateralRepay(vault: PublicKey, owner: PublicKey, loanAmountBaseUnits: number, wallet: Keypair, program: Program<Quartz>): Promise<string> {
//     if (!this.program || !this.wallet || !this.walletWSol) throw new Error("collateralRepayBot is not initialized");
...
//     const collateralRepayDepositPromise = program.methods
...
//     const collateralRepayWithdrawPromise = program.methods
...
//     const collateralRepayStartPromise = program.methods
//         .collateralRepayStart(new BN(walletWsolBalance))
//         .accounts({
//             caller: this.wallet.publicKey,
...
//     ] = await Promise.all([oix_createWSolAtaPromise, collateralRepayStartPromise, jupiterSwapPromise, collateralRepayDepositPromise, collateralRepayWithdrawPromise]);
...
//         this.wSolBank!,
//         this.wSolBank!,
//         [...oix_createWSolAta, ix_collateralRepayStart, ix_jupiterSwap, ix_collateralRepayDeposit, ix_collateralRepayWithdraw],
