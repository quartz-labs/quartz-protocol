...
import { Keypair, PublicKey, SystemProgram, TransactionMessage, VersionedTransaction, Connection, SYSVAR_INSTRUCTIONS_PUBKEY } from "@solana/web3.js";
...
import { getDriftState, getDriftUser, getDriftUserStats } from "../../utils/drift";
import { DRIFT_PROGRAM_ID } from "../../utils/drift";
import { setupTestEnvironment } from "./autoRepaySetup";
...
describe("Quartz Start auto Repay", () => {
    ...
    beforeAll(async () => {
        ...
        await setupDriftAccountWithFundsAndLoan(quartzProgram, banksClient, vaultPda, user);
    });

    test.only("Start auto repay fails when called alone", async () => {
        await makeAutoRepayStartInstructions(quartzProgram, user, 100000000, banksClient);
    });

});
export const makeAutoRepayStartInstructions = async (program: Program<Quartz>, wallet: Keypair, amountLamports: number, banksClient: BanksClient) => {
    ...
    const autoRepayStart = await program.methods
        .autoRepayStart(new BN(amountLamports))
        .accounts({
    ...
    const instructions = [oix_createWSolAta, autoRepayStart];
