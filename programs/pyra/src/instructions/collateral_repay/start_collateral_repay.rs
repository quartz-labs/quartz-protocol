use crate::{
    check,
    config::{PyraError, ANCHOR_DISCRIMINATOR},
...
pub fn validate_instruction_order(
    ...
    // 2nd instruction can be anything, once it's not a Pyra instruction (prevent reentrancy)
    check!(
...
fn validate_drift_markets(
    ...
    check!(
        ...
        PyraError::IdenticalCollateralRepayMarkets
    );
...
fn validate_spl_context<'info>(
    ...
    check!(
        ...
        PyraError::InvalidSourceTokenAccount
    );
