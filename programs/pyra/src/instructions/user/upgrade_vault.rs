use crate::check;
use crate::config::{PyraError, ANCHOR_DISCRIMINATOR, PUBKEY_SIZE};
...
pub fn upgrade_vault_handler(
    ...
    let (vault_owner, vault_bump) = {
        ...
        let owner = Pubkey::new_from_array(
            owner_bytes
                .try_into()
                .map_err(|_| PyraError::FailedToDeserializeVaultBytes)?,
        );
