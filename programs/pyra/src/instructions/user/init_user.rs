use crate::{
    check,
    config::{PyraError, ANCHOR_DISCRIMINATOR, DEPOSIT_ADDRESS_SPACE, INIT_ACCOUNT_RENT_FEE},
    state::Vault,
...
fn init_vault(
    ...
    let space_vault = u64::try_from(Vault::INIT_SPACE).map_err(|_| PyraError::MathOverflow)?;
    create_account(
    ...
    check!(
        ...
        PyraError::InvalidDepositAddressOwner
    );
