...
pub fn get_account_health<'info>(
    ...
    // Pyra health is calculated from initial margin (unlike Drift, which uses maintenance margin)
    ...
    calculate_pyra_account_health(initial_margin_calculation)
}
...
fn calculate_pyra_account_health(initial_margin_calculation: MarginCalculation) -> Result<u8> {
    ...
    let total_collateral_unsigned =
        u128::try_from(total_collateral).map_err(|_| PyraError::MathOverflow)?;
    ...
    let health_u8 = u8::try_from(health).map_err(|_| PyraError::MathOverflow)?;
    Ok(health_u8)
}
